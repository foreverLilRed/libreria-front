...
<main class="flex-grow flex items-center justify-center relative parchment-texture pt-20">
  <div class="absolute inset-0 pointer-events-none opacity-20 overflow-hidden">
    ...
    <div class="absolute bottom-0 right-0 w-full h-full opacity-10">
      <img alt="Blurred library interior" class="object-cover w-full h-full" src="https://lh3.googleusercontent.com/aida-public/AB6AXuAJQ5i0W1JSy93v5s9zVMihCLXs79t4Tm6Q17gzhGvdKuQhfDd0GORB5vsZqBr11z6mt_vpVHZsL6w1N4K57jil6ant_F5p8NsEKLhXxly1XjPLVFKfcMctAisApISmctSUE9KjP4YkMPzJ-GRAxLuEaIW66OJHKlouqTzUzGfgHnKVTkLAUk0L0GoZiNAWz84td93wY7txxEOSwDh6f-Cw2xIA1MLMeMJG9LcocEG66dx9E3tWWTqyS-Gk34F7jue3-EKdbWnKBB_4"/>
    </div>
  </div>
  ...
  <div class="relative z-10 w-full max-w-md px-6">
    <div class="bg-surface-container-lowest/80 glass-card p-10 rounded-xl shadow-[0_40px_80px_-20px_rgba(28,28,25,0.06)] border border-outline-variant/15">
      <div class="mb-10 text-center md:text-left">
        <h1 class="font-headline text-3xl font-extrabold text-primary tracking-tight mb-2">Welcome Back</h1>
        <p class="text-on-surface-variant font-light text-sm">Please authenticate to access the curated archives.</p>
      </div>

      <form class="space-y-8" @submit.prevent="handleLogin">
        <div class="group relative">
          <label class="font-headline text-sm font-semibold text-primary mb-2 block uppercase tracking-wider" for="library-id">Library ID / Email</label>
          <input
            v-model="username"
            ...
            id="library-id"
            name="library-id"
            placeholder="e.g. curator_0821"
            type="text"
          />
        </div>

        <div class="group relative">
          <label class="font-headline text-sm font-semibold text-primary mb-2 block uppercase tracking-wider" for="access-key">Access Key</label>
          <input
            v-model="password"
            class="w-full bg-surface-container-low border-0 border-b border-outline-variant/20 py-3 px-0 focus:ring-0 focus:border-tertiary transition-all duration-300 placeholder:text-stone-300 font-light"
            id="access-key"
        ...
        <div class="pt-4 flex flex-col gap-6">
          ...
          <div class="flex justify-between items-center px-1">
            ...
            <div class="flex items-center gap-1 text-stone-300">
              ...
              <span class="text-[10px] uppercase tracking-tighter">Secure Connection</span>
            </div>
          </div>
        </div>
      </form>
...
<footer class="w-full py-12 px-8 flex flex-col md:flex-row justify-between items-center gap-4 bg-[#f6f3ee] dark:bg-stone-900 border-t border-outline-variant/5">
  <p class="font-inter text-sm font-light tracking-wide text-stone-400 dark:text-stone-500">
    © 2024 The Scholarly Sanctuary. All rights reserved.
  </p>
  <div class="flex gap-8">
    <router-link class="font-inter text-sm font-light tracking-wide text-stone-400 dark:text-stone-500 hover:text-stone-800 dark:hover:text-stone-100 transition-all duration-300" to="#">Privacy Policy</router-link>
    <router-link class="font-inter text-sm font-light tracking-wide text-stone-400 dark:text-stone-500 hover:text-stone-800 dark:hover:text-stone-100 transition-all duration-300" to="#">Terms of Service</router-link>
    <router-link class="font-inter text-sm font-light tracking-wide text-stone-400 dark:text-stone-500 hover:text-stone-800 dark:hover:text-stone-100 transition-all duration-300" to="#">Archive Access</router-link>
  </div>
</footer>
</template>
